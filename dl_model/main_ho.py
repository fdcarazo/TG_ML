...
from model.nn_hyper_param import *
...
def main(config) -> int:
    ...
    if cfg_obj.hyper_optim_apply:  # 2do Hyperparameter Optimization (boolean value: True/False)-.
        if cfg_obj.hyper_optim_name == 'optuna':
            ...
            func = lambda trial: objective(trial, input_size, output_size,
                                           train_loader)
        ...
        best_model = define_model(study.best_trial, input_size, output_size)
        print('{0}{1}Model: {2}{0}'.format('\n', '\t', best_model))
        ...
        with open(filemod, 'wb') as fml: pickle.dump(best_model, fml)

        # to test the model-.
        # X_test= torch.tensor(np.array(X_test.iloc[:,1:]), dtype= torch.float32)  # np.array(X_test.iloc[:,1:])

        if cfg_obj.test:
            # filemod= load_file.root_res+ '/'+ 'dlModel'+ '.pt'
            # load model and scaler-.
            sca = 'scaler'+'.pkl' ## ModelFileName
            scaler_file = load_file.root_res+ '/'+ sca
            with open(scaler_file, 'rb') as fs: scaler= pickle.load(fs)

            # to test the model-.
            X_test = torch.tensor(scaler.transform(X_test.iloc[:,1:]), dtype= torch.float32)  # np.array(X_test.iloc[:,1:])

            model_file = load_file.root_res+ '/'+ 'dlModel'+ '.pt'
            with open(model_file, 'rb') as fm: model= pickle.load(fm)

            with torch.no_grad():
                vals_test= model(X_test)
                print('MSE: {0}{1}'.format(mean_squared_error(vals_test, y_test.iloc[:,:-1]),'\n'))
                print('RMSE: {0}{1}'.format(np.sqrt(mean_squared_error(vals_test, y_test.iloc[:,:-1])),'\n'))
