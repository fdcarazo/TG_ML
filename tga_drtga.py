...
class calc_Deriv():
    ...
    def __init__(self, df, df_list, x_var, y_var, dt, type_biomass):
        ...
        self.df_list = df_list  # list with dataframes, one for each heatRate-.
    ...
    def original_data_sns_regplot(self, dir_save) -> None:
        '''
        plot original data and interpolated values using
        from statsmodels.nonparametric.smoothers_lowess import lowess
        '''
        ...
        fig1.savefig(os.path.join(dir_save+'/figs/',
                                  self.t_b+'_smoothed_with_lowess.png'),
                     format='png', dpi=100)
        ...
        ticks, labels = plt.xticks()
        for idx in range(len(x_list)):
            ax[idx].scatter(x_list[idx], y_list[idx],
                            label='{0}'.format(l_l[idx]),
                            c=l_c[idx], marker=p_s[idx], s=0.7)
            ax_1_1 = ax[idx].twinx()
            ax_1_1.plot(x_smoothed_list[idx],
                        -1.0*lowess_tight_list[idx][:, 1],
                        label=r'smmothed-{0}'.format(l_l[idx]), c=l_c[idx],
                        ls=l_s[idx], lw=3.0, alpha=0.7)
        ...
        fig2.savefig(os.path.join(dir_save+'/figs/',
                                  self.t_b+'_allHR_andDTG_smoothed_with_lowess.png'),
                     format='png', dpi=100)
    ...
    def calc_and_plot_Drtg_dif_approx(self, x, y, npoints):
        '''
        plot derivative of TGA, i.e. DTG (d(TG)/dT) using differents approaches-.
        ...
        # approximations-.
        fun = interpolate.interp1d(x=x, y=y, kind='cubic')
        ...
        ax.scatter(x_toplot, y_cubic, label='cubic_interpolation',
                   c='indianred', marker='<', alpha=0.5)
        # ax.plot(x_toplot, y_cubic, label='interpolated_3_line', c='indianred', ls='--', alpha=0.5)

        ax.scatter(x_toplot, y_splev, label='splev_interp',
                   c='coral', marker='o', alpha=0.5)
        # ax.plot(x_toplot, y_splev, label='splev_interp_line', c='coral', ls='-.', alpha=0.5)
        
        ax.scatter(x_toplot, y_rbi, label='rbi_interp',
                   c='lime', marker='<', alpha=0.5)
        # ax.plot(x_toplot, y_rbi, label='rbi_interp_line', c='lime', ls='-:', alpha=0.5)

        ax.scatter(x_toplot, smoothed_temp_anom, label='witt_smooth',
                   c='deepskyblue', marker='<', alpha=0.5)
        # ax.plot(x_toplot, smoothed_temp_anom, label='witt_smooth_line', c='deepskyblue', ls=':', alpha=0.5)

        ax.scatter(x_toplot, y_savgolFilter, label='savgol_smooth',
                   c='slategrey', marker='8', alpha=0.5)
        # ax.plot(x_toplot, y_savgolFilter, label='savgol_smooth', c='slategrey', ls='':', alpha=0.5)

        ax.legend()
        plt.legend(loc='best')
        ax.set_xlabel('T (K)')
        ax.set_ylabel(r'DTG ($\frac{d(TG)}{dt}$ (mg/s)')

        title = r'DTG ($\frac{d(TG)}{dt}$ (mg/s)'
        ax.set_title(title)
...
#                  index-.
vel_enfr = ['5 Kmin', '10 Kmin', '15 Kmin']  # heatingRate-.
df_names = list()
df_list = list()
for i, i_vel_enfr in enumerate(vel_enfr):
    ...
    # delete the first MultiIndex row:: 'Stalk' y 'Marc'-.y
...
df_stalk_list = [i.iloc[:, 0:3] for i in df_list]
df_marc_list = [i.iloc[:, 3:6] for i in df_list]

calc_deriv_obj = calc_Deriv(df_stalk_list[0], df_stalk_list, 'Temp', 'DrTGA', 1.0, 'Stalk')
## calc_deriv_obj.original_data_sns_regplot(os.getcwd())

calc_deriv_obj = calc_Deriv(df_marc_list[0], df_marc_list, 'Temp', 'DrTGA', 1.0, 'Marc')
## calc_deriv_obj.original_data_sns_regplot(os.getcwd())


# def calc_and_plot_Drtg_dif_approx(self, x, y, npoints):
df = df_marc_list[0].dropna(inplace=True)

# column 0 ==> T (K) - column 1 ==> TGA (mg) - column 2 ==> DrTGA (mg/s)
x = df_marc_list[0]['Temp'].to_numpy().astype(float)
unique, indices = np.unique(x, return_index=True)  # if not I have problems to plot-.
y = df_marc_list[0]['DrTGA'].to_numpy().astype(float)

x = x[indices]
y = y[indices]

# print(np.shape(x), np.shape(x), sep='\n')
# input(33)

calc_deriv_obj.calc_and_plot_Drtg_dif_approx(x, y, 2000)
